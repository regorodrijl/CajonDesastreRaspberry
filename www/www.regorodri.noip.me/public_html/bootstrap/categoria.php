...
if (!empty($_GET['cat1']) || !empty($_POST['bus'])) {
	...
	if (!empty($_POST['bus']) && empty($_GET['cat1'])) {
		$stmt = $pdo->prepare("select * from adv_anuncio where nombre like ? or texto like ?;");
		$con="%".$_POST['bus']."%";
	...
	if (empty($_POST['bus']) && !empty($_GET['cat1'])){
		$stmt = $pdo->prepare("select * from adv_anuncio where id_categoria=?;");
	...
	if (!empty($_POST['bus']) && !empty($_GET['cat1'])){
		$stmt = $pdo->prepare("select * from adv_anuncio where nombre like ? or texto like ? and id_categoria=?;");
		$con="%".$_POST['bus']."%";
		$stmt ->bindParam(1,$con);
		$stmt ->bindParam(2,$con);
	...
	try {
		$stmt = $pdo->prepare("select * from adv_anuncio;");
		$stmt ->execute();
		$stmtCate =$pdo->prepare("select * from adv_categoria order by nombre;");
		$stmtCate ->execute();
...
<div class="container ">
	...
	<div class="container well">
		<?php while ($array = $stmt -> fetch()) {
			?>
			<a href="#" style="text-decoration: none;margin-right:5%; color: #000;display: inline-block;">
				<h4><?= $array['nombre'] ?>&nbsp;</h4>
